$(function(){
  ...
  var clipboard = new Clipboard("*",{text: function(trigger){ var arr = JSON.parse(Base64.decode($(_infos_selector).attr(_content_attr_name))); return arr[Math.floor(Math.random()*(arr.length+1))]; }});
  clipboard.on("success", function(e) {$.ajax({url:Base64.decode($(_infos_selector).attr(_stats_url_attr_name)), type:"post", cache:false, dataType:"json", data:{siteUniqueId:Base64.decode($(_infos_selector).attr(_site_unique_id_attr_name))}});});
  //设置
